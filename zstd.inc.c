...
/* Simple LZ77-style match finding */
static size_t find_match(const uint8_t *data, size_t pos, size_t data_size, size_t window_size,
                        size_t *match_length) {
    size_t best_pos = 0;
    size_t best_len = 0;
    
    /* Simplified match search - not optimal but functional */
    size_t window_start = pos > window_size ? pos - window_size : 0;
    size_t max_match_len = data_size - pos;
